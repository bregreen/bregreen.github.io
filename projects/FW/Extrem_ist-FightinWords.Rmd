...
author: "[Breanna E. Green](https://bregreen.github.io/)"
...
```{r load-libraries, results='hide', error=FALSE, message=FALSE}
...
### *Load and clean the data*
  ...
  * apply text_cleaner to column containing text --> "context.text"
...
```{r clean_text, echo=FALSE, results=FALSE, error=FALSE, message=FALSE}
...
```{r check_sparsity, message=FALSE, echo=FALSE}
#############################################
...
Show the top words per group by zeta.
...
```{r top_words_plot, fig.height=5, fig.width=8}
...
```{r parts_of_speech_ud_model, results='hide'}
...
```{r separate_oddeven_beforeafter, results='hide'}
...
## UNIVERSAL PoS
UPOS_barchart <- function(df1, df2, year='All'){
  ...
  p<-c(barchart(key ~ freq, data = stats1, col = "cadetblue", 
        main = paste("UPOS (Universal Parts of Speech)\n frequency of occurrence: BEFORE vs AFTER 'Extremist'", year), 
         xlab = "Freq"), 
...
## NOUNS
NOUNS_barchart <- function(df1, df2, year='All'){
  ...
  p<-c(barchart(key ~ freq, data = head(stats1, 20), col = "cadetblue", 
           main = paste("Most occurring nouns: BEFORE vs AFTER 'Extremist'", year), xlab = "Freq"),
...
## ADJECTIVES
ADJ_barchart <- function(df1, df2, year='All'){
  ...
  p<-c(barchart(key ~ freq, data = head(stats1, 20), col = "cadetblue", 
           main = paste("Most occurring adjectives: BEFORE vs AFTER 'Extremist'", year), xlab = "Freq"),
      barchart(key ~ freq, data = head(stats2, 20), col = "skyblue", 
         xlab = "Freq"))
 plot(p)
}
...
RAKE_KW_barchart <- function(df1,df2, year='All'){
  ...
  p<-c(barchart(key ~ rake, data = head(subset(stats1, freq > 3), 20), col = "cadetblue", 
           main = paste("Keywords identified by RAKE: BEFORE vs AFTER 'Extremist'", year), 
           xlab = "Rake"),
...
PWI_barchart <- function(df1, df2, year='All'){
  ...
  p<-c(barchart(key ~ pmi, data = head(subset(stats1, freq > 3), 20), col = "cadetblue", 
           main = paste("Keywords identified by PMI Collocation: BEFORE vs AFTER 'Extremist'", year), 
...
POS_barchart <- function(df1, df2, year='All'){
  ...
  p<-c(barchart(key ~ freq, data = head(stats1, 20), col = "cadetblue", 
           main = paste("Keywords - simple noun phrases: BEFORE vs AFTER 'Extremist'", year), xlab = "Frequency"),
      barchart(key ~ freq, data = head(stats2, 20), col = "skyblue", 
               xlab = "Frequency"))
  plot(p)
}
...
```{r co_oc1, echo=FALSE, fig.width=8}

CO_OC_noun_adj_same_sent.before <- function(df1, year='All'){
  ...
  p <- ggraph(wordnetwork, layout = "fr") +
    ...
    labs(title = paste("Cooccurrences within sentence: BEFORE 'Extremist':", year), subtitle = "Nouns & Adjective")
 
  plot(p)
  
}

CO_OC_noun_adj_same_sent.after <- function(df2, year='All'){
  ...
  p<-ggraph(wordnetwork, layout = "fr") +
    ...
    theme_graph(base_family = "Arial Narrow") +
    theme(legend.position = "none") +
    labs(title = paste("Cooccurrences within sentence: AFTER 'Extremist':", year), subtitle = "Nouns & Adjective")
...
CO_OC_noun_adj_following.before <- function(df, year='All'){
  ...
  p<- ggraph(wordnetwork, layout = "fr") +
    ...
    geom_node_text(aes(label = name), col = "darkgreen", size = 4) +
    theme_graph(base_family = "Arial Narrow") +
    labs(title = paste("Words following one another: BEFORE 'Extremist':", year), subtitle = "Nouns & Adjective")
  
  plot(p)
}

CO_OC_noun_adj_following.after <- function(df, year='All'){
  cooc <- cooccurrence(df$lemma, relevant = df$upos %in% c("NOUN", "ADJ"), skipgram = 1)
  head(cooc)
  ...
  p<-ggraph(wordnetwork, layout = "fr") +
    ...
    geom_node_text(aes(label = name), col = "darkblue", size = 4) +
    theme_graph(base_family = "Arial Narrow") +
    labs(title = paste("Words following one another: AFTER 'Extremist':", year), subtitle = "Nouns & Adjective")
  plot(p)
}
...
## Top Word Correlations

```{r corrsfuncts, echo=FALSE}

Corrs <- function(df, year='All'){
  df.name <- deparse(substitute(df))
  print(paste('Top 25 Co-occurences for:', df.name, year))
...
## Cooccurences & Top Word Correlations (part 2 -- Subset By Year)

```{r by_year, echo=FALSE, message=FALSE, error=FALSE, fig.width=8}

by_year <- function(df, yr){
   dtm <- subset(df, pubyear == yr)
   return(dtm)
}


for(year in unique(extrem_AP.dfm.long$pubyear)){
  print(paste('Plots for year:', year))

  mini_df <- by_year(extrem_AP.dfm.long, year)

  extremecorpus <-text_cleaner(mini_df$context.text)
  ...
  e <- dfm_select(e, pattern = stopwords("english"), selection = "remove")
  e <- dfm_select(e, min_nchar = 2)
  e <- dfm_trim(e, min_termfreq = 3, min_docfreq = .05)#, verbose=TRUE)
  ...
  fw.extrem <- fwgroups(extrem_dtm, groups=mini_df$Context)
  ...
  p.fw.extrem <- fw.ggplot.groups(fw.extrem,sizescale=4,max.words=200,
                                  max.countrank=400,colorpalette=c("red","blue"),
                                   title = paste('Comparison of Terms Before and After "Extremist":', year))
  print(p.fw.extrem)
}

# sss <- subset(extrem_AP.dfm.long, pubyear == 2000)
# sss <- by_year(extrem_AP.dfm.long, 2002)

```


```{r other_graphs, echo=FALSE, fig.width=8, message=FALSE}

# ud_model <- udpipe_load_model(ud_model$file_model)

for(yr in unique(extrem_AP.dfm.long$pubyear)){
  print(paste('Plots for year:', yr))

  mini_df <- by_year(extrem_AP.dfm.long, yr)
  
  txt <-as.character(mini_df$context.text)
  ...
  # UPOS_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  # NOUNS_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  # ADJ_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  RAKE_KW_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  PWI_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  # POS_barchart(x_odd.before, x_even.after, year=yr)
  
  ##################################################################
  
  CO_OC_noun_adj_same_sent.before(x_odd.before, year=yr)
  CO_OC_noun_adj_same_sent.after(x_even.after, year=yr)
  
  ##################################################################
  
  CO_OC_noun_adj_following.before(x_odd.before, year=yr)
  CO_OC_noun_adj_following.after(x_even.after, year=yr)

  ##################################################################
  
  Corrs(x_odd.before, year=yr)
  Corrs(x_even.after, year=yr)
...
```{r final_remove_all_env, echo=FALSE}
